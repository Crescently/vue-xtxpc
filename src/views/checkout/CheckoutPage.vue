...
import { getCheckInfo, postCreateOrder } from '@/apis/checkout'
import { useRouter } from 'vue-router'
import { useCartStore } from '@/stores/cart'

const checkInfo = ref({} as checkoutData) // 订单对象
const curAddress = ref({} as address)
const showDialog = ref(false)
const getCheckData = async () => {
  const res = await getCheckInfo()
  checkInfo.value = res.data.result
  //默认 userAddress中的isDefault为 0 做渲染
  curAddress.value = checkInfo.value.userAddresses.find(
    (item: address) => item.isDefault === 0
  ) as address
}
